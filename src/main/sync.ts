...
export function readSyncConfig(workspaceDir?: string): SyncConfig {
  const raw = readJsonFile(getSyncConfigPath(workspaceDir ?? getWorkspaceDir()))
  if (!raw || typeof raw !== 'object') return {}
  return raw as SyncConfig
}

export function writeSyncConfig(patch: Partial<SyncConfig>, workspaceDir?: string): SyncConfig {
  const current = readSyncConfig(workspaceDir)
  const next: SyncConfig = { ...current, ...patch }
  writeJsonAtomic(getSyncConfigPath(workspaceDir ?? getWorkspaceDir()), next)
  return next
}
...
export const SYNC_DIRS = ['data/sessions', 'data/storages', 'data/archived', 'data/profiles', 'skills'] as const
...
export async function git(workspaceDir: string, args: string[]): Promise<{ ok: boolean; stdout: string; error?: string }> {
...
export async function ensureRepo(workspaceDir: string, config: SyncConfig): Promise<string | null> {
  const syncDir = getSyncDir(workspaceDir)
  fs.mkdirSync(syncDir, { recursive: true })
