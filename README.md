...
- 会话支持 **重命名 / 分叉 / 归档 / 移动到分组 / 收藏（★）/ 删除**；分组内会话可一键「**返回工作区**」（移回工作文件夹）。
- 归档按 **年 / 月 / 日** 递归目录存放，打时间与关键词标签，可按时间 / 关键词搜索；归档会话可「**还原到工作区**」（取消归档，回到原工作文件夹与分组位置）。
- **多选批量操作**：日常工作（会话 / 分组）与归档视图均支持多选 + 全选，一键批量删除；删除分组时弹窗可勾选「同时删除分组内全部会话」，不勾选则组内会话自动移回工作文件夹（不丢失）。
...
| R9 | 会话组织增强：返回工作区（分组移出 / 归档还原）、删除分组二次确认（可勾选含会话，不勾选则移回工作文件夹）、会话/分组/归档多选与全选批量删除 | ✅ |
