/**
 * 一键安装 / 一键更新（M2）：
 * - Node.js：从 nodejs.org 获取最新 LTS 版 zip → 解压到 runtime/node → 校验 node.exe；
 * - npm：随便携 Node 提供，单独更新走 npm install -g npm@latest；
 * - pnpm：经便携 Node 的 corepack 启用（失败降级 npm install -g pnpm）；
 * - Git：官方 GitHub Releases（不可达时降级 npmmirror 镜像）→ PortableGit 自解压 → 校验 git.exe；
 * - dsh：npm install @deepseek-ai/dsh --prefix runtime/dsh。
 * 全部产物落在工作文件夹内；安装/更新前备份旧版本；支持取消与实时进度。
 */
...
async function installNode(
  ...
  const version = await fetchLatestLtsVersion(cbs, signal)
...
async function installGit(
  ...
  const downloadsDir = path.join(workspaceDir, 'runtime', '.downloads')
  const action = mode === 'update' ? '更新' : '安装'
  const { version, urls } = await fetchLatestGit(cbs, signal)
  cbs.log(`开始${action} Portable Git ${version}`)

  const sfxPath = path.join(downloadsDir, `PortableGit-${version}-64-bit.7z.exe`)
  const staging = path.join(workspaceDir, 'runtime', `.git-staging-${Date.now()}`)
  try {
    ...
    const gitExe = path.join(staging, 'cmd', 'git.exe')
    ...
    swapWithBackup(staging, gitDir, path.join(workspaceDir, 'runtime', 'git.bak'), 'Git')
    cbs.log(`Portable Git ${check.stdout.trim()} ${action}完成（旧版本已备份至 runtime/git.bak）`)
  } finally {
    cleanup(sfxPath)
  }
}
