...
- 核心工作台仅嵌入 DSH 完整服务 Web，宿主不再改写 DSH DOM；
- 移除客户端模型/API Key 设置、IPC、preload 暴露、适配器和重复凭据同步实现；
- 引导精简为工作文件夹 + 环境检测两步，API Key/模型统一由 DSH 原生服务管理；
- 移除独立会话导航，核心工作台作为唯一 DSH 会话入口；
- 服务自动启动、15 秒 webview 加载兜底、异常状态提示和 30 秒健康检查。

## [2.1.11] - 2026-08-19

### 最终回归加固

- 标题栏版本号从 IPC 动态读取，移除残留的 v2.0.0 占位回退；
- fresh 独立目录启动验收：引导页、主页导航、默认模型、服务启动、端口和个性化配置保护均通过。
