...
async function checkUpdate(): Promise<void> {
  ...
  updateResult.value = `${result.message}（v${result.current}）`
}
  ...
  <div>
    ...
    <section class="mt-5 rounded-lg border border-gray-100 p-4 dark:border-[#23262C]">
      ...
      <div class="mt-4 flex items-center gap-3">
        <el-button size="small" @click="checkUpdate()">检查客户端更新</el-button>
        <span v-if="updateResult" class="text-xs text-gray-500 dark:text-gray-400">{{ updateResult }}</span>
      </div>
