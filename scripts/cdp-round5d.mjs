...
// 工作区树
const tree = await ev(`JSON.stringify([...document.querySelectorAll('aside div')].filter(d => [...d.querySelectorAll('button')].some(b => (b.getAttribute('aria-label')||'') === '工作区操作')).map(r => (r.innerText||'').trim().split(String.fromCharCode(10)).join(' / ')).slice(0,4))`)
...
await wait(800)
const afterExpand = await ev(`(() => { const text = document.querySelector('aside').innerText; const lines = text.split(String.fromCharCode(10)).filter(l => l.trim()); return JSON.stringify({ sessionLines: lines.filter(l => l.indexOf('session-') >= 0 || l.indexOf('新对话') >= 0).slice(0, 5) }) })()`)
