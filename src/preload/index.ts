...
const api = {
  ...
  /** 原子迁移工作文件夹到新路径（流式复制 + 校验 + 切换 + 回滚；过程经 onRelocateEvent 推送）。 */
  relocateWorkspace: (newPath: string): Promise<{ ok: boolean; error?: string; restartRequired?: boolean }> =>
    ipcRenderer.invoke(IPC.WorkspaceRelocate, newPath),

  /** 订阅工作文件夹迁移事件；返回取消订阅函数。 */
  onRelocateEvent: (callback: (event: RelocateEventPayload) => void): (() => void) => {
    const listener = (_event: Electron.IpcRendererEvent, payload: RelocateEventPayload): void => callback(payload)
    ipcRenderer.on(IPC.WorkspaceRelocateEvent, listener)
    return () => {
      ipcRenderer.removeListener(IPC.WorkspaceRelocateEvent, listener)
    }
  },
  ...
  // ---------- 智能同步（Git 时间戳比对） ----------
  syncPreview: (mode?: SyncMode, toleranceMs?: number): Promise<SyncPreviewPayload> =>
    ipcRenderer.invoke(IPC.SyncPreview, mode, toleranceMs),
  syncRun: (input: SyncRunInput): Promise<SyncResultPayload> => ipcRenderer.invoke(IPC.SyncRun, input),
  getSyncSettings: (): Promise<SyncSettingsPayload> => ipcRenderer.invoke(IPC.SyncSettingsGet),
  setSyncSettings: (patch: SyncSettingsPayload): Promise<{ ok: boolean; settings?: SyncSettingsPayload; error?: string }> =>
    ipcRenderer.invoke(IPC.SyncSettingsSet, patch),
  resolveSyncConflict: (rel: string, choice: 'local' | 'remote'): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke(IPC.SyncResolveConflict, rel, choice),
  ...
  // ---------- 会话背景图片（需求五） ----------
  readFileAsDataUrl: (filePath: string): Promise<{ ok: boolean; dataUrl?: string; error?: string }> =>
    ipcRenderer.invoke(IPC.ImageReadDataUrl, filePath),
  saveSessionBackground: (dataUrl: string, name: string): Promise<{ ok: boolean; relPath?: string; error?: string }> =>
    ipcRenderer.invoke(IPC.ImageSaveSessionBg, dataUrl, name),
